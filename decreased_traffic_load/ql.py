...
def packet(lam=0.75):
    ...
    S = SF.copy()  # select 20% of the source-destination pairs
    T = TF.copy()
    real_times = times
    cycle = 0
    ...
    sb, tb = S.copy(), T.copy()
    throughput = []
    while cycle < 100+terminate_cycle:    # simulate for 300 cycles
        rec = 0  # the number of received packets
        send = []  # sending packets for last 2*N+3 cycles
        for i in range(len(mark)):
            send.append(times)
            if cycle > 5 and mark[i] == 0:
                index = len(send) - routing_length(S[i].copy(), T[i].copy())
                temp = int(np.sum(send[index:]))
                if i > temp:  # delay packet
                    mark[i] = -2 if mark[i - temp] != -1 else mark[i]
