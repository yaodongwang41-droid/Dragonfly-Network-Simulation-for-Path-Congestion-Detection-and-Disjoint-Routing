...
def packet(groups=10):
    ...
    drop = 0
    while cycle < 200+terminate_cycle:    # simulate for 280 cycles
        ...
        for i in range(len(mark)):
            if mark[i] != -1:
                if cycle > 220:
                    temp = i - diff -1
                    if temp > 0:
                        s_cycle = 221 + (temp // real_times)
                        # print(times, temp, cycle, s_cycle)
                    else:
                        s_cycle = i // (real_times//groups)
                else:
                    s_cycle = i // times

                if cycle - s_cycle > rl[i%times] + 1:  # drop packets
                    if mark[i] != 0:
