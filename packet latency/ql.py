...
def packet(lam):
    ...
    for p in range(times):
        rl.append(routing_length(S[p].copy(), T[p].copy()))
    cycle = 0
    length = 0
    ...
    while rec < times * 200:
        for i in range(len(mark)):
            index = (rl[i % times] + 1) * times
            if i > index and mark[i] == 0:
                mark[i] = -2 if mark[i - index] != -1 else mark[i]
...
if __name__ == "__main__":
    ...
    file = open('Throughput_ql.txt', 'w')
    for v in z:
        file.write(str(v) + '\n')
    file.close()

    file = open('Received_ql.txt', 'w')
    for v in w:
        file.write(str(v) + '\n')
    file.close()

    file = open('link_utilized_rate_ql.txt', 'w')
    for v in lu:
        file.write(str(v) + '\n')
    file.close()

    file = open('congested_path_ql.txt', 'w')
    for v in u:
        file.write(str(v) + '\n')
    file.close()
